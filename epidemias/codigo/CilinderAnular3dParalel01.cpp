...
int main(){
  ...
  const int tmax=7000007;
  const int ensamble=6666;
...
#pragma omp  for 
      ...
      escupefirst<<numeraauxiliar<<"_TimeCil3dTest.dat"<<std::ends;  
	...
	//recicla Variables
      escupeelotro<<numeraauxiliar<<"_CondInicialesCilindro3d.dat"<<std::ends;         
	...
	for(int t=0; t<tmax; t++){
	  ...
	  if((Uno.DistanciaPlana()<rint)||
	     ( 
	      (DistanciaOrtogonalIntervalo(aux3d, Uno)<rint)
	      &&
	      ((TauOrtogonalIntervalo(aux3d, Uno)<1.0)&&
	       (TauOrtogonalIntervalo(aux3d, Uno)>0.0))))
	      
	    {
	    //Or if punto medio paso por dentro!
	    //  Caminatas<<Uno.x<<"\t"<<Uno.y<<"\t"<<Uno.z<<"\t"<<endl;
	    //Caminatas<<endl;
	    Tiempos<<(t+1)*DeltaT<<endl;
	    break;
	  }
