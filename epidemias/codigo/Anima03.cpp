...
Animalculo Animal1(0.02, 0.0, 0.0);
Animalculo * Animal2;
...
int main(int argc, char* args[]){
  ...
  for(int i=0; i<animales; i++){
    double territorio;
    territorio=sqrt((double)animales)/2;
    //territorio=1.0;

    Animal2[i].x=gsl_ran_flat(r, -0.5, 0.5);
    Animal2[i].y=gsl_ran_flat(r, -0.5, 0.5);
    Animal2[i].CornerX=gsl_ran_flat(r, -territorio, territorio);
    Animal2[i].CornerY=gsl_ran_flat(r, -territorio, territorio);
