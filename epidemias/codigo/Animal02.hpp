...
public:
  ...
  void VueloBrown(gsl_rng * r){
    ...
    */
    ...
    if(abs(x)>bordervalue){    
      cout<<"me pase del borde!"<<endl;
      //creo que por como esta implementado floor
      //en el caso "negativo" las cosas deben funcionar igual...
      if(x>0.00){
	paridad=pow((-1),(floor(x-bordervalue)));
	auxiliar=x-bordervalue-floor(x-bordervalue);
	//	cout<<"aux="<<auxiliar<<endl;
 	x=(bordervalue-auxiliar)*paridad;
	//cout<<"x="<<x<<endl;
      }
      if(x<=0.00){
	auxiliar=floor(x+bordervalue);
    ...
    /*if(abs(y)>bordervalue){
      paridad=(-1)*floor(y+bordervalue);
      y=(y-floor(y+bordervalue))*paridad;
    }
    */
    cout<<x<<endl;
    return;
...
class Animalculo1D{
  /*Los animalculos1d son animales con distancia de interraccion radio.
    Viven en una linea  de lado 1.
    Se mueven en pasos aleatoreos de longitud 
    finura.
    Pueden estar enfermos o sanos.
    Si dos estan a menos de 2 radios de distancia,
...
private:
  const double bordervalue=0.5;
  const double y=0.0; //y interno SIEMPRE es cero
...
public:
  ...
  Animalculo1D(double radio, double CornerX, double CornerY) {
    this->radio=radio;
    x=0;
    enfermo=false;
    this->CornerX=CornerX;
  ...
  Animalculo1D(double radio, double CornerX, double CornerY, double x ) {
    this->radio=radio;
    this->x=x;
    enfermo=false;
    this->CornerX=CornerX;
  ...
  void VueloBrown(gsl_rng * r){
    ...
    */
    ...
    deltax=distancia;
    paridad=gsl_ran_flat(r,0,1.);

    if(paridad<0.5) deltax=-distancia;
    if(paridad>=0.5) deltax=+distancia;
...
double dist(Animalculo &uno, Animalculo1D &dos){
  double result;
  result = (uno.realx()-dos.realx())*(uno.realx()-dos.realx())
    +(uno.realy()-dos.realy())*(uno.realy()-dos.realy());
  result = sqrt(result);
...
void  AbsolutRandomAnimals(Animalculo &primo, Animalculo1D &secundo, gsl_rng *r){
  /*Rutina que les da condiciones iniciales a los Discos
  a partir de la energia y el espacio Disponible
  Think about this...: No es posible poner Ni siquiera el primero donde sea.
  especialmente si son gordos. El centro, por ejemplo, queda excluido.
  Vas a tener que hacer un trial and error. Recuerda que mientras mas
  Gordos sean, mas dificil sera 
  En el caso de que uno sea 1D, es aun mas restrictiva la condicion 
  si su .realy() esta en el centro!
  */ 


  double distmincuad, distcuad; 
  double auxx1,auxx2,auxy1;
  double medioancho=0.5;
  distcuad=0.00;
  distmincuad=(primo.radio+secundo.radio)*(primo.radio+secundo.radio);
  ...
  while(distcuad<distmincuad){
    ...
    //Norma euclidiana al cuadrado
    distcuad=(auxx1-auxx2)*(auxx1-auxx2)+(auxy1)*(auxy1);
