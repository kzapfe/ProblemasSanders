...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.4\textwidth]{DiagramaRadioMaximo.pdf}
...
\begin{equation}
          V_{ds}(r)=32^2b\sqrt{r^2-a^2} -\frac{32
...
The area requires careful treatment of Dirac Delta operators. In our first example
 ...
 % \end{equation}
  ...
  \begin{align}
   ...
   \bigl[ 2ab-\sqrt{2}(ay+bx)+xy \bigr]
    \delta (\sqrt{2x^2+2y^2}-2r).
    \end{align}
  Now the tricky part, we change to polar coordinates as we did in \ref{VolApen},
  and then we perform a second change of variable:
  \begin{align}
    x^2+y^2 =: \rho   \Rightarrow &  \delta(\sqrt{2x^2+2y^2}-2r) \rightarrow
    \delta(\sqrt{2}\rho-2r)\\
    \sqrt{2}\rho-2r =: \xi   \Rightarrow &
    \delta(\sqrt{2}\rho-2r)\rho\rd \rho \rightarrow \delta(\xi)\frac{\rd \xi (\xi+2r)}{2}
    \end{align}
  Then we can perform the integration over $\xi$ and then over the angular $\theta$.
    \begin{align}
    A_\text{col}/4 & =\iint 
    \rd \theta \rd \xi  
   \bigl[ 2ab-\sqrt{\xi+2r}(a\sin\theta+b\cos\theta)+\frac{(\xi+2r)^2}{2}\cos\theta\sin\theta \bigr]
   \delta(\xi)\frac{\rd \xi (\xi+2r)}{2}\\
     & =\int 
    \rd \theta   
   \bigl[ 2abr-2r^2(a\sin\theta+b\cos\theta)+2r^3\cos\theta\sin\theta \bigr]
   \delta (\xi)\\
   A_\text{col} &= 32ab\theta+32r(a\cos\theta-b\sin\theta)+16r^3\sin^2\theta \Big|_\beta^\alpha
    \end{align}
