...
Unhappily, there is a quirk in the above formula:
it stops being valid before that. The reason is that the deduction
of the formula doesn't include the case in which hopping ina one or both
directions is not longer possible. 
We tacitly made the assumption that $h,w>4r$.  This assumption is on 
the integration limits for the $X,Y$ variables in the step in eq. \ref{integraltotal}. 
...
We notice that there are cases which depend on the geometry
of the table. Without loss of generality, we assume that $w\leq h$,
so that horizontal hopping can occur even when vertical hopping
is excluded. That case is divided also in two more cases: if
$ w \leq  h < 2 w $ there is a value for $r$ in which no more hopping is possible,
but the discs still fit inside. For $ 2 w \leq h $, horizontal hopping is
possible until $ 2 r= w$.

We cite the  result for $w/4  <r< h/4$ for illustrative purposes.
For readability we define another auxiliary variable,
$c=\sqrt{4r^2-a^2}$:
\begin{multline}\label{VolumenCasoFeo}
V_{w/4<r<h/4} = 32abr^2[\arccos(a/2r)-\arccos(b/2r))]\\
+\frac{64 r^3}{3 }[b((a-b)/2r)-a(c/2r+\sqrt{4r^2-b^2}/2r)]\\
-8r^4 [ (a^2-b^2)/4r^2]\\ 
+16[ b a^2 c (4\sqrt{2}-1-\sqrt{2}/3)
+c^2a^2 (\sqrt{2}/3-1) \big]
\end{multline}
In the case that $r$ is larger than both $h/4, w/4$, one must take
into account a similar
contribution which inverts the roles of $a$ and $b$. This is only
possible in the case that $h\leq 2 w$.  
...
\section*{Appendix A: Detailed calculations of Volume and Area}

Okey, here things get nasty.
The first thing we need to recognize is that there is freely avaible
volume on the configuration space beyond at least one hoping possibility, and
we must take into consideration two cases. Again, whithout loss
of generality, we can assume that $w \leq h$ always.
Then, we extend the volume formula in eq. \ref{volumewhd}. As
we will see, this results in a continous (as expected) expression,
but without continous second derivative. 
