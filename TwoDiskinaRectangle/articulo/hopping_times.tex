...
\end{figure}
%%%%%%% I would remove the hatched areas and change the caption to "Due to symmetry, we evaluate the integral for X,Y,x,y>0 and multiply by 16." I think it's not hard for people to understand that the integral can be simplified with the symetry and the image is overloaded as it is making it harder to understand the excluded set. 
...
%%%%%% Here did Munukata and Hu obtained only V_\text{free} = V_\text{prism} - V_\text{cyl}, or V_\text{free} =  16 a^{2} b^{2}  - 16 \pi a b r^{2} + \textstyle \frac{64}{3} (a+b) r^{3}  - 8 r^{4} also ??? From the way the text is written it seems that V_\text{free} =  16 a^{2} b^{2}  - 16 \pi a b r^{2} + \textstyle \frac{64}{3} (a+b) r^{3}  - 8 r^{4} was obtained by Munukata and Hu 
...
%%%% Estoy un poco confundida de cual de los hoppings se excluye primero, asi que no se si como lo puse es correcto. 
...
\label{VolMonteC}%% 
\end{figure}
%%% Is it possible to change the legend from numerical to numerical integration?
...
%after taking account of the delta, is
...
is still positive (except in the degenerate case that $w=2h$). %% por que es degenerate este caso? 
...
\end{figure}
%%%%% Specify how you obtained the numeric results... 
...
\label{CasosIntegra}
\end{figure}
%%%%% No entiendo bien la imagen, hay shaded area y area con puntos? Seria bueno mencionar que pasa en el area con puntos. Tambien creo que podriamos los casos como 1, 2, 3, o a), b), c) en la lista y en la imagen para hacer las cosas mas faciles para lxs lectorxs. Ademas en la imagen las desigualdades estan en terminos de a, b y r en la imagen y en terminos de w, h, y r en la lista lo que es innecesariamente confuso. 
