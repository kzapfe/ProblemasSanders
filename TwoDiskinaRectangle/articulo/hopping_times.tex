...
the function to a coordinate that is orthogonal to the surface of section, then
the Jacobian Factor is one. 
...
the $\delta(y_1-y_2)$ operator describes a different function than $\dirac(y)$. If we think of this operators as the limit of smooth functions, the first approaches
the limit in a non orthogonal manner, while the former does. So the first one
introduces a factor on the evaluation, in this
 simple case it is $1\sqrt{2}$. 
This is not obvious until one does the detailed calculations:
...
\end{equation}
\end{widetext}
The extra factor represents the fact that the Dirac Delta is not in a system which
guaranties orthogonality to the surface of section. The best coordinates to represent
the condition for vertical, hopping are the $y,Y$. This can be better understood
in the diagram on figure \ref{transDelta}.
