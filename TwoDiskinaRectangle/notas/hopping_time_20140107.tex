...
% A more general result can be obtained for the cases in which the
% radius of the disks is too large in relation to ``a'' or ``b'',
% thus making impossible a 
% change of positions in the vertical or horizontal position.
% The case in which both hopps are excluded is of no
% interest to us, so we only give here the formula for one of 
% the sides of the box being to small. The excluded volumen
%% Do not know if this could be of interest to anyone.
...
the wedge at the cylinder's end. After derivating we evaluate it again at $d/\sqrt{2}$.
This gives the area as:
\begin{equation}\label{AreaChoque}
A_{collision}(d/\sqrt{2}) =[8\cdot \sqrt{2}  \pi ab] d
-[8 \cdot  \sqrt{2} (a+b)] d^2
+ [4 / \sqrt{2}] d^3.
\end{equation}
Of course, in terms of $h,d$ this is a bit more complex:
\begin{equation}\label{AreaChoquehw}
A_{collision}(d/\sqrt{2}) =[2\cdot \sqrt{2}  \pi (w-d)(h-d)] d
-[4 \cdot  \sqrt{2} (w+h-2d)] d^2
+ [4 / \sqrt{2}] d^3.
\end{equation}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{AreaFraccionTeoyNum01.pdf}
...
\subsubsection{Area of cross section for other impacts}
...
It is simpler then to write the formula in terms of the radius,
which automatically takes this into account.
Other apparent dificulty arises because our
...
intuitive grasp can be obtained by applying the trick to an anulus:
...
is the perimeter. The formula for the avaible space, eq.
\ref{volumewhd} decreases as $d$ increases, so that
we will end then with ``negative areas''. Lastly,
we must take into account that this is the formula
for a geometrical figure whose parameter of size is $d/\sqrt{2}$,
not $d$, because of the coordinate tranfsformation. 

 We start with formula \ref{volumewhd}
 and substitute $d/\sqrt{2}\rightarrow x$ and consider it a function of $x$.
\begin{multline}\label{volumenx}
 V(x)= (w-x\sqrt{2})^2(h-x\sqrt{2})^2
-2\pi(w-x\sqrt{2})(h-x\sqrt{2})x^2 \\
+\frac{8\sqrt{2}}{3}(h+w-2\sqrt{2}x)x^3
-2 x^4
\end{multline}
