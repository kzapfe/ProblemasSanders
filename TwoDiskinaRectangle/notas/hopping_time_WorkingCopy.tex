...
\affiliation{ Also Facultad de Ciencias? Current afiliation?}
...
billiard.  We have shown that the gneral formula is appliable in a very broad range
of situations and the only dificulty is establishing the adecuate surface
of section and its measure, so that it represents  the event in question. 
We have compared this results against
numerical simulations and we have compared them also agains first event times.
The second case has no closed expresion that we are aware of, but shows
very similar behavior, altough it has clear deviations.   
\end{abstract}
...
inertial motion and compare them with first hopping and collition times.
...
the later follows closely the first, but with apreciable
cualitative diferences. 
...
the set of equation \ref{cambiocoor01}. Then the exlusion condition
...
The substitutions $a\rightarrow (w-d)/2$ and $b\rightarrow (h-d)/2$ give us
 the Volume as function from the diameter only:
...
the disks is discussed in supplementary material \footnote{Appendix A can be 
found on blablabla.}. 
