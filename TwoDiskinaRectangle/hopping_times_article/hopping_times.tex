...
The simplest such system that is still non-trivial is that of two discs in a box.
...
for both inertial and Brownian dynamics \cite{Bowles04}, although they focus
on the \emph{first-passage} time, that is, the mean time at which events
...
a finite number of disjoint ergodic components (two or four), which are each these still hyperbolic.
...
Eq.~\eqref{cambiocoor01}, since the coordinates $X$ and $Y$ 
...
is affected by the exclusion condition, and $I_2$ is where the exclusion condition has no effect on that range.
 In figure \ref{diagintegral01} this would correspond to the doted and
...
\begin{figure}[h]
  ...
  \caption{Mean disc collision time as function of the radius. Same
...
\begin{figure}[h]
        ...
        \caption{The three possible cases. The shaded area is where the integral
          ...
          allows only for vertical hopping; and the last case excludes hopping, but it is
        is still possible for the discs to fit inside the table.}
\label{CasosIntegra}
\end{figure}
