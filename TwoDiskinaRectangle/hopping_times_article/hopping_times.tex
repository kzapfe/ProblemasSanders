...
\begin{figure}[h]
  ...
  \caption{The space to integrate is the product of the spaces
    ...
    point of evaluation
    on the other subspacewhich can range from no width at
    all to $r\sqrt{2}$.
    Due to 
...
intuiton, but the leading term is exactly $w^2(h/2-w/4)^2$, 
and the denominator
stays the same, so
it goes to infinity
like $1/x^2$. The figure \ref{MeanHopp01} shows the compariton between
...
Do simplify the formulas, we do not write explicitly the dependence
of $a$ and $b$ on $r$.

An implicit assumption was made on the limits of integration in \eqref{integraltotal}.
