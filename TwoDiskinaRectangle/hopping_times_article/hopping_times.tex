...
with an increase in algebraic difficulty. It would also be possible to extend the results to calculate exact mean collision
and hopping times for systems containing more discs, but the analytical calculations become more challenging \cite{three_hard_discs_2004}.
In the limit of a large number of hard spheres, Chernov \cite{Chernov97} was able to apply these methods to derive the exact limiting  free flight time for a gas of hard spheres.

%Adding one more disk can be a challenging problem, although
...
The cross-sectional areas are calculated by fattening the surface $g(\mathbf{x}) = 0$ to $g(\mathbf{x}) \le \epsilon$, or, in the
case of the hopping cross-section, $|g(\mathbf{x})| \le \epsilon$.
