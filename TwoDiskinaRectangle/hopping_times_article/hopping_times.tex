...
\begin{itemize}
\item Horizontal and vertical hopping possible: $0 <r \leq h/4$ ($r<b$);
\item Only vertical hopping possible: $h/4 < r \leq w/4$ ($b<r<a$);
\item No hopping possible: $w/4 < r < (h+w - \sqrt{2hw}) / 2$ ($a,b<r$).
\end{itemize}
...
\begin{figure*}[h]
        ...
        \begin{subfigure}[b]{0.32\textwidth}
          ...
          \label{Caso3}
        \end{subfigure}%
        \caption{Three hopping regimes: (a) all hopping is possible, (b) only vertical hopping is possible and (c) no hopping is possible. The excluded space includes the hatched circular sectors and the dotted triangular areas.}
\label{CasosIntegra}
\end{figure*}
The hatched and dotted areas in Fig.~\ref{CasosIntegra} represent the region in which the indicator function
has value 1. 

We evaluate the integral by carefully choosing the limits of integration. The hatched area in all three cases may be conveniently integrated in polar coordinates $(\rho, \theta)$.
